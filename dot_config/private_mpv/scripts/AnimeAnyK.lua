...
function sendAnime4kCommand_jbgyampcwu()
  ...
  local modeACommand = restoreCnnPath
    .. upscaleCnnX2Path
  ...
  function getAnime4KFullCommand(primaryModeString, debugText)
    ...
    primaryModeString = commandPrefixConst
      .. '"'
      .. primaryModeString
      .. '"'
      .. commandShowTextConst
      .. '"'
      .. commandShowTextContentConst
      .. debugText
      .. '"'
...
function videoLoadedEvent_jbgyampcwu(event)
  ...
  if indicatorFileExist == false then
    return
  else
...
function inputCommandEvent_jbgyampcwu()
  ...
  if indicatorFileExist == false then
    ...
    local closeResult, err = pcall(function()
      file_object:close()
    end)
    ...
    local deleteResult, err = pcall(function()
...
function runFuntionButDelay_jbgyampcwu(myTime, myFunction)
  mp.add_timeout(myTime, function()
    myFunction()
  end)
end
